...
base_info:
    __author__ = PyGo
    __time__ = 2025/11/30 13:56
    __version__ = v.1.0.0
    __mail__ = gaoming971366@163.com
    __blog__ = www.pygo2.top
    __project__ = fastslot-api
    __file_name__ = error.py
...
async def error_http_exception_404(
    ...
    """
    自定义FASTAPI-HTTPException异常处理 > HTTP_404_NOT_FOUND
    :param exec: 是否触发异常
    :return: json
    """
    if exec:
        raise FASTAPI_HTTPException(
            status_code=fastapi_http_status.HTTP_404_NOT_FOUND,
...
async def error_http_exception_500(
    ...
    """
    自定义FASTAPI-HTTPException异常处理 > HTTP_500_INTERNAL_SERVER_ERROR
    :param exec: 是否触发异常
    :return: json
    """
    if exec:
        raise FASTAPI_HTTPException(
            status_code=fastapi_http_status.HTTP_500_INTERNAL_SERVER_ERROR,
