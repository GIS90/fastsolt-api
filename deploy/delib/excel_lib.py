...
base_info:
    __author__ = PyGo
    __time__ = 2025/12/6 19:57
    __version__ = v.1.0.0
    __mail__ = gaoming971366@163.com
    __blog__ = www.pygo2.top
    __project__ = fastslot-api
    __file_name__ = excel_lib.py
...
class ExcelLib:
    ...
    def font_content(cls):
        ...
        __font = xlwt.Font()
        __font.name = 'Times New Roman'
        __font.bold = True
        __font.color_index = 4
        __font.height = 220
        style_title.font = __font
        return style_title
    ...
    def font_title(cls):
        ...
        __font.color_index = 4
        __font.height = 220
        style_content.font = __font
        return style_content

    def read_headers(self, excel_file: str) -> Dict:
        """
        ...
        """
        res: Dict = {'sheets': {}, 'nsheet': 0, 'names': {}, 'columns': {}}
        if not excel_file \
    ...
    def get_real_file(self, file_name: str, _type=1) -> Tuple[str, str]:
        """
    ...
    def get_split_store_dir(self, dir_name: str) -> str:
        """
    ...
    def merge_openpyxl(self, new_name: str, file_list: list, **kwargs) -> Dict:
        ...
        # 分割行数,默认为0
        blank: int = kwargs.get('blank') or self.blank
        try:
            ...
            for f in file_list:
                ...
                sheet_names = reader_excel.sheetnames
                max_nsheet = int(f.get('nsheet')) if f.get('nsheet') else len(sheet_names)   # 获取表格最大Sheet数，防止index超出
    ...
    def merge_xlrw(self, new_name: str, file_list: list, **kwargs) ->Dict:
        ...
        # 分割行数,默认为0
        blank: int = kwargs.get('blank') if kwargs.get('blank') else self.blank
        try:
